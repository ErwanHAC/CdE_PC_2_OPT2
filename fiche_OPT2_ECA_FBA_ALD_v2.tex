...
\begin{enonce}
	...
	\reponse{$\cos^2 a = \frac{1 + \cos \left( 2a \right)}{2} $}
...
\begin{enonce}
	...
	\reponse{$\sin^2 a = \frac{1 - \cos \left( 2a \right)}{2} $}
...
\reponse{$\frac{2\pi}{\omega_2}$}

% \begin{corrige}
% 	La pulsation du signal $s_2(t) = S_2 \sin \left( \omega_2 t - k_2 x + \varphi_2 \right)$ est $\omega_2$ donc sa période est $T_2 = \frac{2\pi}{\omega_2}$.
% \end{corrige}
...
% \begin{enonce}
% $s_5\left( t \right) = \left( s_1(t) + s_2(t) \right)^2 $
% \end{enonce}
...
% \begin{enonce}
% 	$s_6\left( t \right) = \left( s_1(t) + s_2(t) \right)^2 $ avec $\omega_1 = \omega_2$.
% 	\end{enonce}
...
% 	\end{align*}
% 	On constate que $\sin\left( 2 \omega_1 \left(t+\tau\right) - 2 k_1 x \right) = \sin\left( 2 \omega_1 \left( t+10^6 \times T_3 \right) - 2 k_1 x \right) = \sin\left( 2 \omega_1 t - 2 k_1 x \right)$, avec $T_3 = \frac{2\pi}{2\omega_1}$, donc
% 	$$
...
La différence de marche entre les deux rayons 1 et 2 vaut : $\delta_{\ptS \ptM}=\mathcal{L}_{\ptS\ptM,2}-\mathcal{L}_{\ptS\ptM,1}=\mathcal{L}_{\mathcal{M}_2 \ptL\ptS_2} - \mathcal{L}_{\ptL\ptS_1 \mathcal{M}_1}$.
